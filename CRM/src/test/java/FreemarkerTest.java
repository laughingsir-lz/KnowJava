...
public class FreemarkerTest {
    ...
    public void test() throws  Exception{
        ...
        String domainName = "Notice"; //domain类名
        root.put("chinese","公告通知管理"); //中文,用于权限注解
        //------------------------------------------------------
        root.put("capitalize",domainName); //大写开头
        root.put("uncapitalize", StringUtils.uncapitalize(domainName)); //小写开头
        String packageDir = "src/main/java/com/lz/ssm"; //项目目录
        // //--------------------controller控制器---------------
        Template temp = cfg.getTemplate("controller.ftl");
        // 设置输出为新的文件
        Writer out = new OutputStreamWriter(new FileOutputStream(packageDir+"/controller/"+domainName+"Controller.java"));
